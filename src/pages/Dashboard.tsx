...
export default function Dashboard() {
  ...
  const handleEditClick = (analysis: AnalysisSummary) => {
    ...
    setEditSessionName(
      analysis.session_name || `Session du ${new Date(analysis.date).toLocaleDateString("fr-FR")}`
    );
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        if (compareSlot1 === analysisToDelete) {
          setCompareSlot1("");
          setCompareResult1(null);
        }
        if (compareSlot2 === analysisToDelete) {
          setCompareSlot2("");
  ...
  const isBestScore =
    featuredAnalysis && featuredScore >= statistics.bestScore && statistics.bestScore > 0;

  const featuredGrade = featuredAnalysis?.performance_score?.grade || latestAnalysis?.grade || "C";
  const featuredSessionName =
    featuredAnalysis?.session_conditions?.session_name || latestAnalysis?.session_name;
  ...
  return (
    <Layout>
      ...
      <PageMeta
        ...
        description="Historique de vos analyses et comparaison de sessions."
        path="/dashboard"
      />
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {featuredAnalysis && !currentFolderId && (
          ...
          >
            <div className="rounded-2xl p-4 sm:p-6 md:p-8 border-2 border-primary/40 bg-primary/5 relative overflow-hidden group shadow-2xl shadow-primary/5">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-end">
                <div>
                  <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-1">
                    {featuredSessionName ||
                      featuredAnalysis?.session_conditions?.circuit_name ||
                      latestAnalysis?.circuit_name ||
                      "Session"}
                  </h3>
                  <p className="text-muted-foreground text-sm md:text-lg mb-6 uppercase tracking-widest flex items-center gap-2">
                    ...
                    {(featuredAnalysis.session_type || "practice") === "practice"
                      ? "ESSAIS"
                      : (featuredAnalysis.session_type || "practice") === "qualifying"
                        ? "QUALIF"
                        : (featuredAnalysis.session_type || "practice") === "race"
                          ? "COURSE"
                          : (featuredAnalysis.session_type || "practice").toUpperCase()}
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 md:gap-4 max-w-2xl">
                    <Button
                      ...
                      onClick={() => handleViewAnalysis(featuredAnalysis.analysis_id)}
                    >
                    ...
                    <div className="flex gap-2 flex-1">
                      <Button
                        ...
                        onClick={() => handleCompareAdd(featuredAnalysis.analysis_id)}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => handleDeleteClick(featuredAnalysis.analysis_id)}
                      >
                ...
                <div className="h-40 sm:h-48 md:h-64 bg-secondary/10 rounded-2xl border border-white/5 p-2 relative overflow-hidden group-hover:border-primary/20 transition-colors">
                  <TrackMap
                    ...
                    laps={
                      ...
                      featuredAnalysis.plot_data.trajectory_2d.laps.length > 0
                        ? [
                            featuredAnalysis.plot_data.trajectory_2d.laps[featuredBestLapIdx] ||
                  ...
                  <div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-background/80 to-transparent flex items-end justify-center pb-2 pointer-events-none">
                    <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-[0.2em] opacity-50">
                      Aperçu du tracé
                    </span>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12 opacity-70 group hover:opacity-100 transition-opacity">
          ...
          <div className="bg-secondary/20 p-4 rounded-xl border border-white/5 text-center">
            ...
            <div className="text-xl md:text-3xl font-bold text-foreground text-primary">
              {statistics.bestScore}/100
            </div>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {currentFolderId && (
              <Card
                className="glass-card p-6 flex flex-col items-center justify-center cursor-pointer border-dashed border-white/10 hover:border-primary/50 transition-all hover:bg-white/5 active:scale-95"
            ...
            {visibleFolders.map((folder) => (
              <Card
                key={folder.id}
                className="glass-card p-6 cursor-pointer hover:border-primary/50 transition-all group relative hover:translate-y-[-4px]"
              ...
              >
                ...
                <p className="text-sm text-muted-foreground">
                  {folder.analysisIds.length} analyses
                </p>
                <div className="flex gap-2 mt-4">
                  <Badge
                    variant="secondary"
                    className="bg-white/5 text-[9px] py-0 px-2 uppercase border-none"
                  >
                    Practice
                  </Badge>
                  <Badge
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-1">
            <div
              className={`flex-1 w-full h-14 md:h-16 rounded-2xl border-2 flex items-center justify-between px-4 md:px-6 transition-all ${compareSlot1 ? "border-primary bg-primary/5 shadow-inner" : "border-dashed border-white/10 bg-secondary/10"}`}
            >
              <span
                className={
                  compareSlot1
                    ? "font-bold text-foreground text-sm md:text-base truncate pr-2"
                    : "text-muted-foreground italic text-xs md:text-sm"
                }
              >
                {compareSlot1
                  ? (() => {
            ...
            <div
              className={`flex-1 w-full h-14 md:h-16 rounded-2xl border-2 flex items-center justify-between px-4 md:px-6 transition-all ${compareSlot2 ? "border-primary bg-primary/5 shadow-inner" : "border-dashed border-white/10 bg-secondary/10"}`}
            >
              <span
                className={
                  compareSlot2
            ...
            <Button
              variant="hero"
              className="w-full md:w-40 h-14 md:h-16 md:ml-4 rounded-2xl font-bold shadow-xl shadow-primary/10 mt-2 md:mt-0"
              disabled={!compareSlot1 || !compareSlot2}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 md:hidden">
            {visibleAnalyses.map((analysis) => (
              <Card
                key={analysis.id}
                className="glass-card p-4 flex flex-col gap-4 border-white/5 hover:border-primary/50 transition-all"
                onClick={() => handleViewAnalysis(analysis.id)}
              >
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-bold text-foreground text-lg">
                      {formatDate(analysis.date).split(" à")[0]}
                    </div>
                    ...
                    <div className="flex gap-2 mt-1">
                      <Badge
                        variant="outline"
                        className="text-[8px] uppercase border-white/10 bg-white/5 py-0 px-1.5"
                      >
                        {(analysis.session_type || "practice") === "practice"
                          ? "Essais"
                          : (analysis.session_type || "practice") === "qualifying"
                            ? "Qualif"
                            : (analysis.session_type || "practice") === "race"
                              ? "Course"
                              : analysis.session_type || "Essais"}
                      </Badge>
                      <Badge
                        variant="outline"
                        className="text-[8px] uppercase border-white/10 bg-white/5 py-0 px-1.5"
                  ...
                  <div className="text-right">
                    ...
                    <Badge className={`${getGradeColor(analysis.grade)} border-none text-[10px]`}>
                      {analysis.grade}
                    </Badge>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/5">
                  <div className="font-mono text-foreground font-bold text-sm">
                    Meilleur : {analysis.lap_time.toFixed(2)}s
                  </div>
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleMoveAnalysisRequest(analysis.id);
                      }}
                    >
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-9 w-9 hover:bg-red-500/20 hover:text-red-500"
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  {visibleAnalyses.map((analysis) => (
                    ...
                    >
                      ...
                      <TableCell>
                        ...
                        >
                          <SelectTrigger
                            className={`h-7 px-3 text-[10px] uppercase border-white/10 w-[90px] ${
                              analysis.session_type === "race"
                                ? "bg-red-500/20 text-red-300"
                                : analysis.session_type === "qualifying"
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className="text-[10px] uppercase border-white/10 bg-white/5"
                        >
                          {folders.find((f) => f.analysisIds.includes(analysis.id))?.name ||
                            "Racine"}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <Badge
                            className={`${getGradeColor(analysis.grade)} border-none text-[10px]`}
                          >
                            {analysis.grade}
                          </Badge>
                        </div>
                      ...
                      <TableCell className="text-right pr-6">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Button
                            ...
                            title="Comparer"
                            className="h-10 w-10 hover:bg-primary/20 hover:text-primary"
                            onClick={() => handleCompareAdd(analysis.id)}
                          >
                          ...
                          <Button
                            ...
                            className="h-10 w-10 hover:bg-primary/20 hover:text-primary"
                            onClick={() => handleMoveAnalysisRequest(analysis.id)}
                          >
                          ...
                          <Button
                            ...
                            size="icon"
                            title="Supprimer"
                            className="h-10 w-10 hover:bg-red-500/20 hover:text-red-500"
                ...
                <div className="sticky top-0 z-10 p-4 md:p-6 flex items-center justify-between border-b border-white/5 bg-background/80 backdrop-blur-xl">
                  <div className="flex items-center gap-2 md:gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] md:text-sm text-muted-foreground uppercase tracking-widest truncate max-w-[150px] md:max-w-none">
                        {selectedAnalysis.session_conditions?.session_name ||
                          selectedAnalysis.session_conditions?.circuit_name ||
                  ...
                  <div className="flex items-center gap-2 md:gap-4">
                    <Badge
                      className={`${getGradeColor(selectedAnalysis.performance_score.grade)} border-none py-1 px-2 md:py-1.5 md:px-4 text-[10px] md:text-sm font-black`}
                    >
                      {selectedAnalysis.performance_score.grade} (
                      {getDisplayScore(selectedAnalysis.performance_score)})
                    </Badge>
                    <Button
                      variant="outline"
                      size="sm"
            ...
            >
              <div className="p-4 md:p-6 flex items-center justify-between bg-secondary/50 border-b border-white/5 backdrop-blur-xl">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-10 w-10 md:h-12 md:w-12 rounded-full border border-white/10"
        ...
        <Dialog open={isMoveDialogOpen} onOpenChange={setIsMoveDialogOpen}>
          <DialogContent className="glass-card">
            ...
            <DialogFooter>
              <Button variant="ghost" onClick={() => setIsMoveDialogOpen(false)}>
                Annuler
              </Button>
              <Button variant="hero" onClick={handleConfirmMove}>
                Déplacer
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="glass-card">
            ...
            <DialogFooter>
              ...
              <Button variant="hero" onClick={handleSaveSessionEdit}>
                Enregistrer
              </Button>
            </DialogFooter>
          </DialogContent>
