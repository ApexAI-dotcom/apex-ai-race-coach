...
export const AlertBanner = ({ profile, recent_sessions = [] }: AlertBannerProps) => {
  ...
  if (profile.engine_hours_life && profile.engine_hours_current !== undefined) {
    ...
    if (ratio >= critLimit)
      alerts.push({ level: "critical", message: "Moteur : Révision haut-moteur imminente." });
    else if (ratio >= warnLimit)
      alerts.push({ level: "warning", message: "Moteur : Prévois une révision prochainement." });
  }
  ...
  if (profile.tires_sessions_life && profile.tires_sessions_current !== undefined) {
    ...
    if (ratio >= critLimit)
      alerts.push({ level: "critical", message: "Pneus : En fin de vie, prévoir un train neuf." });
    else if (ratio >= warnLimit)
      alerts.push({ level: "warning", message: "Pneus : Baisse d'adhérence probable." });
  }
  ...
  if (profile.brakes_sessions_life && profile.brakes_sessions_current !== undefined) {
    ...
    if (ratio >= critLimit)
      alerts.push({ level: "critical", message: "Freins : Contrôle les plaquettes urgemment." });
  }
